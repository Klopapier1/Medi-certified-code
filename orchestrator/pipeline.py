...
from . import ckg, evidence, roles, tools
...
class Pipeline:
    ...
    def _run_requirements(self) -> str:
        ...
        (self.spec.output_dir / "requirements" / "SRS.md").write_text(text, encoding="utf-8")
    ...
    def _run_design(self, srs: str) -> str:
        ...
        text = _strip_fences(self.client.complete(system, user))
        (self.spec.output_dir / "design" / "SDD.md").write_text(text, encoding="utf-8")
    ...
    def _run_code(self, sdd: str) -> str:
        ...
        (self.spec.output_dir / "src" / f"{self.spec.name}.py").write_text(code, encoding="utf-8")
    ...
    def _run_static_analysis(self) -> None:
        ...
        (self.spec.output_dir / "evidence" / "static-analysis-report.md").write_text(
    ...
    def _run_tests(self, srs: str, code: str) -> None:
        ...
        (self.spec.output_dir / "tests" / f"test_{self.spec.name}.py").write_text(
        ...
        (self.spec.output_dir / "evidence" / "test-report.md").write_text(report, encoding="utf-8")
    ...
    def _run_risk(self, srs: str, sdd: str) -> None:
        ...
        text = _strip_fences(self.client.complete(system, user))
        (self.spec.output_dir / "evidence" / "risk-table.md").write_text(text, encoding="utf-8")
    ...
    def _compile_evidence(self) -> None:
        ...
        (self.spec.output_dir / "evidence" / "traceability-matrix.md").write_text(
        ...
        ckg.write_graph(self.spec.output_dir, self.spec.name)

    def _write_signoff_template(self) -> None:
        path = self.spec.output_dir / "evidence" / "review-signoff.md"
        if not path.exists():
            path.write_text(SIGNOFF_TEMPLATE.format(name=self.spec.name), encoding="utf-8")
